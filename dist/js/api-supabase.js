/* ==========================================================================
   CLIDANFI · api-supabase.js  ·  IMPLEMENTACIÓN REAL CON SUPABASE
   --------------------------------------------------------------------------
   CÓMO ACTIVARLO (3 pasos):

   1) Ejecuta `supabase/schema.sql` en el SQL Editor de tu proyecto.

   2) En index.html, ANTES de este archivo, añade el cliente oficial:
        <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
      y DESPUÉS de js/api.js añade:
        <script src="./js/api-supabase.js"></script>
      (este archivo sobrescribe window.API con la versión real)

   3) Rellena SUPABASE_URL y SUPABASE_ANON_KEY aquí abajo.

   La firma de cada función es IDÉNTICA a la de js/api.js, así que las vistas
   no cambian ni una línea.
   ========================================================================== */
(function (global) {
  'use strict';

  /* --- Credenciales: SIEMPRE desde js/env.js, nunca escritas aquí -------- */
  const ENV = global.CLIDANFI_ENV || {};
  const SUPABASE_URL = ENV.SUPABASE_URL || '';
  const SUPABASE_ANON_KEY = ENV.SUPABASE_ANON_KEY || '';

  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    console.warn('[CLIDANFI] Sin credenciales en js/env.js → modo demostración (localStorage).');
    return;
  }
  if (!global.supabase || typeof global.supabase.createClient !== 'function') {
    console.error('[CLIDANFI] Falta el cliente de Supabase (js/vendor/supabase.js). Ejecuta: npm install');
    return;
  }
  ...
  const auth = {
    ...
    salir: async () => { await sb.auth.signOut(); },

    sesion: async () => {
      const { data } = await sb.auth.getSession();
      return _normalizar(data.session);
    },

    onCambio: (cb) => {
      const { data } = sb.auth.onAuthStateChange(async (_evento, session) => cb(await _normalizar(session)));
  ...
  /* ======================================================================
     ...
     ====================================================================== */
  // Aquí NO hay guardia en cliente: la autoridad es RLS, en el servidor.
  // Cualquier consulta fuera de lugar vuelve vacía o con error de política.
  global.API = {
    _impl: 'supabase', sb, auth, miPaciente,
    getConfig, setConfig,
